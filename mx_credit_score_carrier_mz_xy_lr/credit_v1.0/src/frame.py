...
tag 0   1   bad_rate
score_cut           
(350, 500]  246 644 0.723596
(500, 550]  471 502 0.515930
(550, 600]  809 453 0.358954
(600, 650]  923 288 0.237820
(650, 700]  420 74  0.149798
(700, 950]  68  6   0.081081

score_line≥650 pass_rate = 12% bad_rate = 15% 
...
def mx_scores(y_predprob):
    ...
    scores = 683.91 + 72.13*np.log(np.abs((1- y_predprob))/y_predprob)
    ...
    df['grade'] = df['score'].apply(score_to_grade)
    df['decision'] = df['score'].apply(lambda x : 'REJECT' if x < 650 else 'APPROVE')
...
def model_preditc(df,model,feature_final,LOG,mx_scores):
    try:
        feature_final_woe = [col + '_woe' for col in feature_final]
        y_predprob = model.predict_proba(df[feature_final_woe])[:, 1]
