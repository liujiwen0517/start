...
@app.route('/api/carrier_mz/risk/score/v1.0',methods=['POST'])
def access_model_predict():
    start_time = datetime.now()
    data_dict = request.get_json(force=True)
    data_df = pd.DataFrame(data_dict,index=[0])
    try:
        feature_final = load_IndependModel(path,'feature_selected',LOG)
        result,errorMsg,returnCode = predict_flask(data_df,'xgb_30_v1.0',feature_final,path,load_IndependModel,LOG,mx_scores,errorMsg = '')
